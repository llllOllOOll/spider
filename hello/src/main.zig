...
pub fn main(init: std.process.Init) !void {
    ...
    _ = app.get("/products", listProducts).listen() catch |err| return err;
}
...
fn listProducts(allocator: std.mem.Allocator, req: *Request) !Response {
    ...
    const mock_products = [_]MockProducts{
    ...
    for (mock_products) |p| {
        ...
        item.* = Context.init();
        try item.set(allocator, "name", p.name);
        try item.set(allocator, "price", p.price);
        try items.append(allocator, item);
    }

    var context = Context.init();
    defer context.deinit(allocator);
    try context.setList(allocator, "products", items);

    const tmpl = Templates{};
    const html = try spider.template.renderWith(tmpl.list_products, &context, allocator, tmpl);
