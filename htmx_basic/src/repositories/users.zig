...
pub const UserRepository = struct {
    ...
    pub fn create(self: UserRepository, input: model.CreateUserInput) !model.User {
    ...
    pub fn findById(self: UserRepository, id: u64) !?model.User {
    ...
    pub fn findByEmail(self: UserRepository, email: []const u8) !?model.User {
    ...
    pub fn findAll(self: UserRepository) ![]model.User {
        ...
        if (count == 0) return &[_]model.User{};

        const users = try self.allocator.alloc(model.User, count);
    ...
    pub fn update(self: UserRepository, id: u64, input: model.UpdateUserInput) !?model.User {
        ...
        return .{ .id = id, .email = new_email, .name = new_name };
    }
